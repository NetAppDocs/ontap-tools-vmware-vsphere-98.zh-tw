...
[role="lead"]
如果您在ONTAP 單一VMware vSphere HTML5用戶端管理多個vCenter Server執行個體的環境中、使用VMware vSphere的VMware vSphere工具、則必須在每個vCenter Server上登錄一個VSC執行個體、以便VSC與vCenter Server之間進行一對一配對。如此一來、您就能從單一vSphere HTML5用戶端、以連結模式和非連結模式、同時管理執行vCenter 6.0或更新版本的所有伺服器。
